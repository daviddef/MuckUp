...
final class BrisbaneEventsService {
    ...
    /// Radius used both to scope the API query and to decide whether it's
    /// even worth calling — Brisbane-only dataset, so anyone far away
    /// (e.g. Melbourne, where the rest of Grub's mock data lives) gets
    /// nothing rather than a pile of irrelevant interstate results.
    ...
    func fetchNearby(_ location: CLLocation) async -> [PartnerItem] {
        ...
        components.queryItems = [
            URLQueryItem(name: "where", value: #"event_type="Green" and start_datetime>"\#(today)" and distance(geolocation, geom'\#(point)', \#(searchRadiusMetres)m)"#),
